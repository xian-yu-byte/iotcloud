...
public interface ICMapper {
    ...
    //添加projectadd的数据
    int AddProject(ProjectAdd projectAdd);

    //查询用户名是否存在
    users selectUserByUsername(String username);

    //注册用户
    int RegisteredUser(users user);
    ...
    List<UserProject> selectByUserId(@Param("userid") Integer userid);
    ...
    // 删除用户-项目关联记录（退出项目）
    int deleteUserProject(@Param("userid") Integer userid, @Param("projectid") Integer projectid);

    // 根据项目ID查询项目
    List<ProjectAdd> selectJoinedProjectsByUser(@Param("userid") Integer userid);

    // 根据用户ID和项目ID查询用户-项目关联记录
    UserProject selectUserProjectByUserIdAndProjectId(@Param("userid") Integer userid,
                                                      @Param("projectid") Integer projectid);

    // 根据项目ID查询项目
    ProjectAdd selectProjectById(@Param("id") Integer id);
}
